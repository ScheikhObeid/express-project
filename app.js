...
const processFilesSequentially = async (directoryName, interval) => {
    ...
    // Get the list of files and sort them for consistent processing order
    const files = await fs.readdir(processDir).catch(() => []);
    files.sort(); // Optional: Ensure consistent order if filenames have sequential numbering
    ...
    if (files.length === 0) {
        ...
        delete processes[directoryName];
        try {
            await fs.rm(processDir, { recursive: true }); // Delete directory if empty
            console.log(`Directory "${processDir}" deleted successfully.`);
        } catch (deleteError) {
            console.error(`Failed to delete directory "${processDir}":`, deleteError.message);
        }
...
app.post('/start', async (req, res) => {
    ...
    if (!APIEndpoint || typeof APIEndpoint !== 'string') {
        return res.status(400).send({ error: 'API Endpoint is required and must be a string.' });
    }
