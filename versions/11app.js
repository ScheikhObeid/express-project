...
// API endpoints
// const apiUrls = [
//     'https://traffics-stage.ameropa.de/railhub/',
...
const appendResponseToFile = async (directoryName, responseData) => {
    const processDir = path.join('./responses', directoryName); // Use directoryName for responses
...
const sendRequest = async (directoryName, url, requestData) => {
    ...
    axios.post(url, requestData, {
        ...
        .then(async (response) => {
            ...
            console.log(`Response from ${url} (Directory: ${directoryName}):`, response.data);

            // Write response directly to a file
            await appendResponseToFile(directoryName, {
        ...
        .catch(async (error) => {
            const endTime = Date.now();
            console.error(`Error from ${url} (Directory: ${directoryName}):`, error.message);
...
const processFilesAtInterval = async (directoryName) => {
    ...
    if (files.length === 0) {
        console.log(`No files to process in directory: ${directoryName}`);
        // clearInterval(processes[directoryName].intervalId); // Stop interval if no files
        // return;
                // Use stopProcess function to stop the process
                const result = stopProcess(directoryName, processes);

                if (result.success) {
                    console.log(`Process for directory "${directoryName}" complated successfully.`);
                    ...
                    console.error(result.error); // Log any issues if stopping the process fails
                }
    ...
    for (const file of files) {
        ...
        const success = await processRequestFromFile(directoryName, filePath);
...
function stopProcess(directoryName, processes) {
    ...
    const process = processes[directoryName]; // Access the process details using directoryName

    if (!process) {
        return { success: false, error: `No process found for directory: ${directoryName}` };
    }

    // Clear the interval and remove the process from the map
    clearInterval(process.intervalId);
    delete processes[directoryName];

    console.log(`Process for directory "${directoryName}" stopped.`);
    return { success: true, message: `Process for directory "${directoryName}" stopped successfully.` };
}
...
//     if (!process) {
//         return res.status(400).send({ error: `No process found for directory: ${directoryName}` });
//     }
...
//     console.log(`Process for directory "${directoryName}" stopped.`);
//     res.send({ message: `Process for directory "${directoryName}" stopped successfully.` });
// });
